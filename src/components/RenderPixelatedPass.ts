  ...
  material() {
    return new THREE.ShaderMaterial({
      uniforms: {
        ...
        tDepth: { value: null },
        tNormal: { value: null },
        normalEdgeStrength: { value: 0.5 }, // Added uniform
        depthEdgeStrength: { value: 0.4 }, // Added uniform
        resolution: {
...
// Helper function to create a pixelated render target
function pixelRenderTarget(
  ...
  const renderTarget = new WebGLRenderTarget(resolution.x, resolution.y, {
    depthTexture: depthTexture
      ? new THREE.DepthTexture(resolution.x, resolution.y)
      : undefined,
